...
#pragma warning restore 0414
    ...
    public static bool WaitForTestSetup = true;

    public static int Main(String[] args)
    {
//		while (WaitForTestSetup)
//		{
////I guess we just chew CPU on the tiny profile for now :facepalm:
//			Thread.Sleep(100);
//		}
//		is_attached = Debugger.IsAttached;
//		try
//		{
//			args = File.ReadAllLines("TestArgs.txt");
//		}
//		catch (FileNotFoundException)
//		{
//			// If we don't find the file, assume we are debugging the debugger test app
//			// and just continue. The user should have passed the command line args via the
//			// debugger.
//		}
        ...
        invoke();
        exceptions();
        exception_filter(); // Uses dynamic code generation
        threads();
        dynamic_methods(); // Uses dynamic code generation
        user();
        type_load();
        regress();
        gc_suspend();
        ...
        local_reflect();
        call_frame_tests();
        call_use_pinned_ptr_byref_local();
        call_generic_method();
        if (args.Length > 0 && args[0] == "domain-test") /* This takes a lot of time, so execute it conditionally */
            domains();
        if (args.Length > 0 && args[0] == "ref-emit-test")
            ref_emit(); // Uses dynamic code generation
    ...
    {
        ss_recursive2_at("ss_recursive2 in " + x.ToString());
        if (x < 5)
        {
            int next = x + 1;
            ss_recursive2_at("ss_recursive2 descend " + x.ToString());
            ss_recursive2_trap();
            ss_recursive2(next);
        }

        ss_recursive2_at("ss_recursive2 out " + x.ToString());
    }
...
//		Console.WriteLine (indent + (exiting ? "<--" : "-->") + " " + at + " " + n);
    ...
    */
    ...
    public static void vtypes()
    {
        ...
        t.boxed_struct_field = new KeyValuePair<int, object>(1, (long)42);
    ...
    {
        return s2[0] + s3[0].ToString() + s4[0, 0].ToString();
    }
    ...
    public static void exceptions()
    {
        ...
        nested_exceptions(0);
        nested_exceptions(1);
        catch_different_exception_types(0);
        catch_different_exception_types(1);
        catch_different_exception_types(2);
    }
    ...
    [MethodImpl(MethodImplOptions.NoInlining)]
    public static void nested_exceptions(int level)
    {
        string message;
    ...
    class ThirdException : SecondException
    {
    ...
    [MethodImpl(MethodImplOptions.NoInlining)]
    public static void catch_different_exception_types(int level)
    {
        string message;
        int i = -1;
    ...
    public static void call_frame_tests()
    {
        var test = new Tests();
        test.method_with_local();
    }

    public void method_with_local()
    {
        int local = 5;
        increment(ref local);
    }
    ...
    public void method_to_invoke() { }

    public unsafe static void call_use_pinned_ptr_byref_local()
    {
        use_pinned_ptr_byref_local(null, null, 0, 0);
    }

    public unsafe static int use_pinned_ptr_byref_local(byte* input, byte* output, int inputLength, int outputLength)
    {
        var hashTable = new byte*[64];
        fixed (byte** entry = &hashTable[0])
        {
            return LZ4_compressCtx_32(entry, input, output, inputLength, outputLength);
        }
    }

    unsafe static int LZ4_compressCtx_32(byte** reference, byte* input, byte* output, int inputLength, int outputLength)
    {
        return 42;
    }

    public static void call_generic_method()
    {
        generic_method_that_returns_its_argument(42);
    }

    private static T generic_method_that_returns_its_argument<T>(T value)
    {
        int i = 0;
        i++;
        return value;
